...
const QGCWaypointTable: React.FC<QGCWaypointTableProps> = ({ waypoints, onDelete, onUpdate, activeWaypointIndex }) => {
  const headers = ['#', 'Frame', 'Command', 'P1', 'P2', 'P3', 'Distance', 'Lat', 'Long', 'Alt', 'Actions'];

  const handleValueChange = (
    id: number,
    field: keyof Omit<Waypoint, 'id' | 'command'>,
    value: string
  ) => {
    const numericValue = value === '' ? 0 : parseFloat(value);
    if (isNaN(numericValue)) return; // Ignore non-numeric strings like "abc"
    ...
    if (field === 'param4' && previousWaypoint) {
      ...
      // Standard update for any other field
            ...
            {waypoints.length === 0 ? (
                <tr>
                    <td colSpan={headers.length} className="text-center py-8 text-gray-500">
                        Upload a mission file or click 'Edit Plan' and click on the map to begin.
                    </td>
                </tr>
            ) : (
              waypoints.map((wp, index) => {
                ...
                return (
                  <tr key={wp.id} className={`hover:bg-gray-800 ${isActive ? 'bg-green-800/50' : ''}`}>
                    ...
                    <td className="px-2 py-1"><input type="number" value={wp.frame} onChange={(e) => handleValueChange(wp.id, 'frame', e.target.value)} className="bg-gray-700 w-12 p-1 rounded" /></td>
                    <td className="px-2 py-1">
                      <select 
                        value={wp.command}
                        onChange={(e) => handleCommandChange(e, wp.id)}
                        className="bg-gray-700 border-gray-600 rounded p-1 text-sm focus:outline-none focus:ring-1 focus:ring-green-500"
                      >
                    ...
                    <td className="px-2 py-1"><input type="number" value={wp.param1 || 0} onChange={(e) => handleValueChange(wp.id, 'param1', e.target.value)} className="bg-gray-700 w-12 p-1 rounded" /></td>
                    <td className="px-2 py-1"><input type="number" value={wp.param2 || 0} onChange={(e) => handleValueChange(wp.id, 'param2', e.target.value)} className="bg-gray-700 w-12 p-1 rounded" /></td>
                    <td className="px-2 py-1"><input type="number" value={wp.param3 || 0} onChange={(e) => handleValueChange(wp.id, 'param3', e.target.value)} className="bg-gray-700 w-12 p-1 rounded" /></td>
                    <td className="px-2 py-1">
                      <input 
                        type="number" 
