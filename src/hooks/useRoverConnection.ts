...
export const useRoverConnection = () => {
    ...
    const commandResponseListeners = useRef<((response: any) => void)[]>([]);

    const addCommandResponseListener = useCallback((callback: (response: any) => void) => {
        commandResponseListeners.current.push(callback);
        return () => {
            commandResponseListeners.current = commandResponseListeners.current.filter(
                (cb) => cb !== callback
            );
        };
    }, []);
    ...
    const connect = useCallback(() => {
        ...
        socket.on('command_response', (data: { status: 'success' | 'error', message: string }) => {
            ...
            commandResponseListeners.current.forEach((callback) => callback(data));
        });
    ...
    return { connectionStatus, roverData, connect, disconnect, sendCommand, addCommandResponseListener };
};