...
const MapView: React.FC<MapViewProps> = ({
  ...
  const invalidateAndFitBounds = () => {
    if (!mapRef.current) return;
    setTimeout(() => {
      if (!mapRef.current) return;
      mapRef.current.invalidateSize();
      if (missionWaypoints.length > 0) {
        const missionBounds = L.latLngBounds(missionWaypoints.map(wp => [wp.lat, wp.lng]));
  ...
  }, [viewMode, isFullScreen, isMapFullScreen]);
  ...
  useEffect(() => {
    ...
    if (missionWaypoints.length > 0) {
      ...
      missionWaypoints.forEach((wp, index) => {
        const icon = getWaypointIcon(wp, index, missionWaypoints.length, activeWaypointIndex);
  ...
  }, [missionWaypoints, viewMode]);

  useEffect(() => {
    if (!mapRef.current || missionMarkersRef.current.length === 0) return;
    missionWaypoints.forEach((wp, index) => {
      if (missionMarkersRef.current[index]) {
        missionMarkersRef.current[index].setIcon(getWaypointIcon(wp, index, missionWaypoints.length, activeWaypointIndex));
      }
    });
  }, [activeWaypointIndex]);
  ...
  useEffect(() => {
    if (!mapRef.current || !roverPosition || missionWaypoints.length < 1) return;
    const missionBounds = L.latLngBounds(missionWaypoints.map(wp => [wp.lat, wp.lng]));
    if (missionBounds.isValid()) {
      const boundsToFit = missionBounds.extend(L.latLng(roverPosition.lat, roverPosition.lng));
      mapRef.current.fitBounds(boundsToFit.pad(0.2), { animate: true, duration: 0.5, maxZoom: 22 });
    }
  }, [roverPosition]);
  
  useEffect(() => {
      if (!mapRef.current) return;
      const nextWaypoint = missionWaypoints.find(wp => wp.id === activeWaypointIndex);
  ...
  }, [roverPosition, activeWaypointIndex, missionWaypoints]);
  ...
  return (
    <div ref={mapWrapperRef} className="relative w-full flex-1 rounded-lg overflow-hidden bg-gray-700 flex flex-col">
       ...
       <div className="absolute top-2 right-2 z-[1000] flex flex-col gap-2">
         ...
         {missionWaypoints.length > 0 && (<button onClick={invalidateAndFitBounds} className="p-2 bg-gray-800 bg-opacity-70 rounded-md text-white hover:bg-opacity-90" title="Auto Zoom & Pan to mission"><ExpandIcon className="w-5 h-5" /></button>)}
       </div>
